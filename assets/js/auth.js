    ...
    $('#btnlogin').click(function(){
        ...
        if ($('#username').val() != '' && $('#password').val() != '') {
            ...
            var password = $('#password').val();
                $.ajax({
                    url: "http://localhost/CarwashBookingSystem/app/auth/auth.php?login",
                    method: "POST",
    ...
    $('#btnregist').click(function(){
        ...
        if ($('#firstName').val() != '' && $('#lastName').val() != '' && $('#inputUsername').val() != '' && $('#inputPhone').val() != '' && $('#inputEmail').val() != '' && $('#inputPassword').val() != '') {
            ...
            var lastName = $('#lastName').val();
                $.ajax({
                    url: "http://localhost/CarwashBookingSystem/app/auth/auth.php?regist",
                    method: "POST",
                    data: {username:username, password:password, firstname:firstName, lastname:lastName, email:email, phone:phone}, 
                    success:function(data){
                        console.log(data);
                        if (data == 0 ) {
                        $('#btnregist').html('');
                        $('#btnregist').html('Sign up');
                            ...
                            alertLogin += 'Akun sudah berhasil di daftarkan.';
                            alertLogin += '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>';
                            alertLogin += '</div>';
